    ...
    timeInterval:(NSTimeInterval)timeInterval
{
    if(!_pause) {
        if(_interval > 1.0 / _fps) {
            _frame += (NSInteger)floor(_interval / _fps);
    ...
    [drawable drawOnContext:context object:object cameraHalfSize:cameraHalfSize cameraCenter:cameraCenter depth:depth timeInterval:timeInterval];
}
